...
def convert_mask_multiclass(mask_list):
    """
    Convert RGBA masks to palette-based indexed images for 4 classes + background.
    ...
    for i in mask_list:
        ...
        # Map RGBA values to class indices
        for y in range(height):
            for x in range(width):
                r, g, b, a = pixels[x, y]
                
                # Determine class based on RGB values
                if a == 0 or (r == 0 and g == 0 and b == 0 and a == 0):
                    # Background (transparent or black with no alpha)
                    class_idx = 0
                    ...
                    # Default to background for unrecognized colors
                    class_idx = 0
                
                out_pixels[x, y] = class_idx
        
        # Create palette: 5 colors (background + 4 classes)
        palette = make_palette("light","green", "yellow","dark","red")  # Adjust if needed for 5 classes
        out.putpalette(palette)
        
        out.save(i)
    
    print("Masks converted to multi-class labels, please check for correctness")
...
if __name__ == "__main__":
    ...
    files_target, files_source = load_img(target_path, source_path)
    convert_mask_multiclass(files_target)
